    ...
    , m_window(window)
{
    ...
    // NB: Upstream removed the 16x16 PNG icon set; use a Qt standard icon instead.
    banner_icon->setPixmap(style()->standardIcon(QStyle::SP_MessageBoxWarning).pixmap(24, 24));
    ...
    tab_layout->addWidget(m_autofill_blocked_banner);
    ...
    // NB: Upstream removed the 16x16 PNG icon set; use a Qt standard icon instead.
    m_network_audit_action->setIcon(style()->standardIcon(QStyle::SP_DriveNetIcon));
    ...
    // Fork additions: Tor, VPN, and Network Audit toolbar buttons
    navigation_button_layout->addWidget(create_toolbar_button(*navigation_button_cluster, *m_tor_toggle_action));
    navigation_button_layout->addWidget(create_toolbar_button(*navigation_button_cluster, *m_vpn_toggle_action));
    navigation_button_layout->addWidget(create_toolbar_button(*navigation_button_cluster, *m_network_audit_action));
